...
// cuda에 device 세팅 함수, device 번호와 CUDA에서 현재 지정한 device를 비교.
...
// CUDA 동적 메모리 할당, windows 환경에선 폴백 구조를 지원하지 않아 에러 처리 후 바로 cudaMalloc로 넘어감
...
// cuda 초기화 함수, 디바이스 정보와 vram 계산(cuda로 gpu의 정보를 얻어오고, cpu가 gpu의 자원 파악을 하기 위함)
...
// 레거시 풀, n개의 버퍼풀이 있다면 n개의 버퍼길이만큼 탐색하므로 O(n^2)일 가능성. 하지만 이 수준에서 cuda를 손보기 보단 고수준의 
// 파일에서 최적화하는 것이 더욱 중요.
...
// vmm 풀, cu_check 에 크기 검사를 맡기므로 조금 불안정해 보임.
...
// 아마 호출하는 함수에 따라 연산 기본 틀은 같지만 내부 로직이 다른걸 보면 다른 라이브러리로 계산하는듯함.
// 아래 cuda_mul에선 cublas를 호출하지 않음.
...
// ggml 쿠다 행렬 연산 함수, 연산할 컨텍스트, 소스 텐서 0, 1, 연산 결과 dst, op는 어떤 타입의 곱인지 추정.
// 마지막 파라미터는 양자화에 대한 정보 파라미터로 추정
static void ggml_cuda_op_mul_mat(
    ...
    // 4~9는 왜 없는지 모르겠음, 아마 nrow를 뽑아낸 것과 관련이 있지 않나 싶음.
    const int64_t ne10 = src1->ne[0];
    ...
    // 따로 텐서의 값을 빼두는 이유가 있을 것

    // 버퍼가 비어있는지 비어있지 않은지 체크
    GGML_ASSERT(ggml_backend_buffer_is_cuda(dst->buffer));
    GGML_ASSERT(ggml_backend_buffer_is_cuda(src1->buffer));

    // 버퍼 체크 후 컨텍스트를 계산하기 위해 따로 포인터값 저장
    ggml_backend_cuda_buffer_context * src1_ctx = (ggml_backend_cuda_buffer_context *) src1->buffer->context;
    ggml_backend_cuda_buffer_context * dst_ctx  = (ggml_backend_cuda_buffer_context *) dst->buffer->context;
    
    // 왜 ne0X 변수들을 쓰지 않았는가? bool 결과값만 원하면 그래도 됐을건데.
    ...
    // 에러 핸들링 or 범위 계산에 들어갈 size 계산인듯?
    ...
    for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
        ...
        // 양자화 버전일 시 가벼운 연산을 위해 실행되는 듯 보임
        if (quantize_src1) {
    ...
    // 메인 디바이스와 보조 디바이스들의 교착 | 데드락상태 처리 함수들로 보임.
}
// 병렬 처리용 포인터로 보임, 글로벌화로 모든 스레드에서 한 작업시 전체 변수를 보게 하기 위함인듯?
