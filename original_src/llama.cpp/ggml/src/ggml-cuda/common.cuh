...
// GPGPU 언어 선택 HIP / MUSA / 둘 다 아니라면 C base CUDA
...
// 전처리 bool 함수는 템플릿 메타 프로그래밍으로 처리한 듯 보임
...
// 미리 오프셋들 선언해둔 상태값들 판단해서 에러핸들링을 처리한 것으로 보임
...
// 런타임에 GPU 아키텍쳐에서 확인이 필요하기 때문에 템플릿 메타 프로그래밍을 쓰지 않음
...
// constexpr 로 물리적 크기를 미리 계산하고 상수처리해야 하므로 사용.
...
// 컴파일러가 컨티뉴가 존재하는 반복문 최적화를 하지 못 할때, 재귀적으로 루프를 푸는 템플릿 코드
